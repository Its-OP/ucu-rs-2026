...
def test_perfect_ranking():
    ...
    # arrange
    ranked_item_ids = np.array([10, 20, 30])
    true_ratings = {10: 5.0, 20: 4.0, 30: 3.0}

    # act
    result = ndcg_at_k(ranked_item_ids, true_ratings, k=3)
...
def test_reversed_ranking():
    ...
    # arrange
    ranked_item_ids = np.array([30, 20, 10])
    true_ratings = {10: 5.0, 20: 4.0, 30: 3.0}
...
def test_single_rated_item_at_top():
    # arrange
...
def test_single_rated_item_not_at_top():
    # arrange
    ranked_item_ids = np.array([2, 1, 3])
    true_ratings = {1: 5.0}
...
def test_k_larger_than_ranked_list():
    ...
    # act
    result = ndcg_at_k(ranked_item_ids, true_ratings, k=10)
...
def test_k_truncates_ranked_list():
    ...
    # arrange
    ranked_item_ids = np.array([10, 20, 30, 40])
    true_ratings = {10: 5.0, 20: 4.0, 30: 1.0, 40: 5.0}
    ...
    # act
    result = ndcg_at_k(ranked_item_ids, true_ratings, k=2)

    # assert
    assert result == pytest.approx(dcg / idcg)


def test_unknown_items_have_zero_gain():
    """Items not in true_ratings contribute 0 gain."""
    # arrange
    ranked_item_ids = np.array([99, 1])
    true_ratings = {1: 5.0}
    expected = (5 / math.log2(3)) / (5 / math.log2(2))
...
def test_graded_gains_distinguish_ratings():
    """A 5-star item at rank 1 should score higher than a 3-star item at rank 1."""
    # arrange
    true_ratings = {1: 5.0, 2: 3.0}

    # act
    ndcg_good = ndcg_at_k(np.array([1, 2]), true_ratings, k=2)
    ndcg_bad = ndcg_at_k(np.array([2, 1]), true_ratings, k=2)
...
def test_accepts_list_input():
    ...
    # arrange
    ranked_item_ids = np.array([1, 2, 3])
    true_ratings = {1: 5.0, 2: 4.0, 3: 3.0}
...
def test_hand_computed_value():
    ...
    # arrange
    ranked_item_ids = np.array([3, 1, 2])
    true_ratings = {1: 5.0, 2: 4.0, 3: 2.0}
