...
### 3.3 Relevance Binarization for Ranking Evaluation
 
Since our metrics are ranking-based, we need to define what counts as a **relevant** item in the val/test set. We apply a threshold:
...
**Definition.** For a given user, let `rel_i` be the binary relevance of the item at position `i` in the model's ranked list:
 
```
DCG@K  = Σ (i=1..K)  rel_i / log2(i + 1)
IDCG@K = DCG@K for the ideal ranking (all relevant items first)
NDCG@K = DCG@K / IDCG@K
```
 
The final NDCG@K is averaged across all users in the evaluation set.
 
**Why primary.** NDCG is position-sensitive: placing a relevant item at rank 1 contributes far more than placing it at rank 10. This directly models user behavior — users inspect the top of the list first and attention decays rapidly. Among ranking metrics, NDCG is the most informative single number because it rewards both the *presence* and the *ordering* of relevant items.
...
**Definition.** Fraction of a user's relevant items that appear in the top-K list:
...
| Metric | Type | What it measures | Sensitive to position? |
|---|---|---|---|
| **NDCG@K** | Primary | Ranking quality with position discount | Yes |
| **Precision@K** | Secondary | Purity of the top-K list | No (set-based) |
| **Recall@K** | Secondary | Coverage of user's relevant items | No (set-based) |
...
| **Relevance threshold** | Rating >= 4 | Separates genuine positive signal from lukewarm in a positively-skewed distribution (mean 3.58) |
| **Primary metric** | NDCG@10 | Position-aware ranking quality |
